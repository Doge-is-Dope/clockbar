...
- `App/` — SwiftUI App, view model, `AppUpdater` (Sparkle wrapper), `AppContainer`, `PunchReminderCoordinator` (decides Late / Missed / Cross-day notifications), `WakeObserver` (subscribes to `NSWorkspace` wake notifications and drives both `viewModel.refresh()` and `coordinator.checkPending(...)`).
